...
export default class SketchCtrl extends ZepetoScriptBehaviour {
    ...
    private puzzlePieces:UnityEngine.GameObject[] = [null,null,null,null,null,null,null,null]; // 0 ?†ÑÏ≤?,1 ?åê, 2 ?ôº?ïÑ?ûò, 3 ?ôº?ñºÍµ? 4 ?ôº?úÑ 5 ?ò§Î•∏ÏïÑ?ûò 6?ò§Î•∏ÏñºÍµ? 7 ?ò§Î•∏ÏúÑ
    private AnswerPosition:UnityEngine.Vector3[] = [null,null,null,null,null,null];
    private CurrentPiece:UnityEngine.GameObject; // ?òÑ?û¨ Ï°∞Í∞Å
    private CorrectPiece:number = 0; // ÎßûÏ∂ò ?çºÏ¶? ?àò
    ...
    Start() {   
        ...
        this.curRoom.AddMessageHandler("UpdatePlayers",(message:string[])=>{
            ...
            ZepetoWorldHelper.GetUserInfo(message,(info:Users[])=>{
                for(let i = 0; i<info.length;i++){
                    this.Profiles[i].transform.GetChild(2).gameObject.GetComponent<TextSize>().SetText(info[i].name);
        ...
        this.curRoom.AddMessageHandler("EndVote",(message:PuzzleModel)=>{
            ...
            this.UIGroup[4].transform.GetChild(0).GetComponent<TextSize>().SetText("Let's assemble the dream shrads to complete a nightmare.");
        ...
        this.curRoom.AddMessageHandler("TotalScore",(message:TotalScore)=>{
            ...
            this.ScoreTexts[0].text = message.total.toString();
            ...
            LeaderboardManager.instance.SendScore(message.total);
        ...
        this.curRoom.AddMessageHandler("RespondScore",(message:string)=>{
            if(this.Bed==null){
                return;
            }
    ...
    *GenerateLogo(){
        ...
        this.TimerText.text = "";
        this.puzzlePieces.forEach((value)=>{
        ...
        for(let i = 2; i<8; i++){
            if(!this.puzzlePieces[i].activeSelf){
                continue;
            }
        ...
        this.curRoom.Send("RequestScore");
