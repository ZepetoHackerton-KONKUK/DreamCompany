...
export default class UICtrl extends ZepetoScriptBehaviour {
    ...
    public TextUI:Text[]; // 0 ?ù¥Î¶? 1 ?Å¨Î¶¨Ïä§?Éà 2 Î≥¥ÎÑà?ä§ 3 Î≥¥ÎÑà?ä§ ?Ç®????åê
    ...
    Start() {  
        ...
        this.curRoom.AddMessageHandler("onGetStorageResult",(message:StorageMessage)=>{
            ...
            if(message.key == "Achievement"){
                ...
                if(this.canAttend && message.value == "7"){
                    value = 0;
                    this.curRoom.Send("onSetStorage",{key:"AttendanceCount",value:"0"} as StorageMessage);
                }
                if(this.canAttend){
        ...
        this.curRoom.AddMessageHandler("SyncInventories",(message:any)=>{
            this.RefreshAllBalanceUI();
        });
        this.curRoom.AddMessageHandler("UpdateTier",(message:number)=>{
            if(this.NpcCanvas == null){
                return;
            }
            while(this.myTier<message){
                this.myTier++;
                this.TierUpReward();
        ...
        this.curRoom.AddMessageHandler("UserInfo",(message:UserInfo)=>{
            ...
            this.remainSoup = message.RemainBonus;
            ...
            ZepetoWorldHelper.GetUserInfo(name,(info:Users[])=>{
                this.TextUI[0].GetComponent<TextSize>().SetText(info[0].name);
            },(error)=>{
                console.log(error);
            });
        ...
        ProductService.OnPurchaseCompleted.AddListener((product,response)=>{
            ...
            if(response.productId == "soup_z"){
                this.StartCoroutine(this.AfterPurchaseSoupbyZem());
            }
            this.RefreshAllBalanceUI();
        });
    ...
    *AfterPurchaseSoupbyZem(){
        ...
        prevdata.Add("quantity",this.remainSoup-3);
    ...
    private *RefreshSoup(){
        ...
        if(this.remainSoup == 0){
            ...
            this.bonusNum = 0;
            this.curRoom.Send("SetBonus",{bonus:0,remain:0});
            this.TextUI[3].text = "";
        }else{
    ...
    private TierUpReward(){
        ...
        this.TierInfo[2].sprite = this.TierNames[this.myTier-1];
        this.TierUpUI.SetActive(true);
        this.ActiveAchievement(this.myTier);
        const frame = this.FrameSelects[this.myTier] as Transform;
        frame.gameObject.GetComponentInChildren<Button>(true).interactable = true;
    ...
    private ActiveAchievement(index:number){
        ...
        Achive.transform.GetChild(1).gameObject.SetActive(false);
    }
    private AquireAchievement(index:number){
        this.AchieveNum[index]="1";
        this.Achievements[index].transform.GetChild(1).gameObject.SetActive(true);
        ...
        const achieveData = new RoomData();
        achieveData.Add("key","Achievement");
        achieveData.Add("value",message);
        this.curRoom.Send("onSetStorage",achieveData.GetObject());
    }
    ...
    private Attend(index:number){
        ...
        const dayData = new RoomData();
        dayData.Add("key","AttendanceDate");
        dayData.Add("value",this.GetDayInfo());
        const attendData= new RoomData();
        attendData.Add("key","AttendanceCount");
        attendData.Add("value",(index+1).toString());
        this.curRoom.Send("onSetStorage",dayData.GetObject());
        this.curRoom.Send("onSetStorage",attendData.GetObject());
        this.RefreshAllBalanceUI();
